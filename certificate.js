...
/* Populate assessor dropdown + the optional right-signatory signature dropdown */
...
function drawCanvas(){
  ...
  /* TÜV stamp — overlaid above/left of the QR, matching the sample.
     Drawn last so it sits on top, with slight transparency like a real stamp. */
  if (assets.stamp) {
    const ss = 96;                 // stamp diameter in pt
    const stx = 410 - ss/2;        // centre x ~410 (over note line / QR top-left)
    const sty = 538;               // top y
    ctx.save();
    ctx.globalAlpha = 0.9;
...
async function exportPDF(){
  ...
  // TÜV stamp overlay (drawn last, on top)
  if (assets.stamp){
    const ss=96, stx=410-ss/2, sty=538;
