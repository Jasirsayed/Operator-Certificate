...
const assets = { bg: null, photo: null, qr: null, stamp: null };
...
const F = {
  ...
  managerSig:  document.getElementById('f_managerSig'),
};

/* Populate assessor dropdown + the right-signatory signature dropdown.
   The right signatory defaults to the first assessor's signature (so a
   signature shows by default, like the sample); choose "None" to omit it. */
ASSESSORS.forEach((a, i) => {
  ...
  if (F.managerSig) {
    ...
    if (i === 0) o2.selected = true;   // default right signature = first assessor
    F.managerSig.appendChild(o2);
  }
});
...
function drawCanvas(){
  ...
  /* Assessor signature (left signatory) — centred above the
     "<assessor> / Assessor" caption. */
  const sig = currentSig();
  if (sig) {
    fitInto(ctx, sig, 268*s, 628*s, 94*s, 46*s);
  }

  /* Optional right signatory signature (signature only, name typed separately) */
  const rsig = currentManagerSig();
  if (rsig) {
    fitInto(ctx, rsig, 451*s, 628*s, 94*s, 46*s);
  }
  ...
  /* QR (smaller now, to make room for the stamp) — centred at x~404 */
  if (assets.qr) {
    const qs = 70;                 // QR size in pt (was ~87)
    const qx = 404 - qs/2;         // centre between the two signatory blocks
    const qy = 618;
    ctx.drawImage(assets.qr, qx*s, qy*s, qs*s, qs*s);
  }

  /* TÜV stamp — smaller, sits in the upper-right of the signatory area
     (above the QR, overlapping only its top-right corner) like the sample. */
  if (assets.stamp) {
    const ss = 74;                 // stamp diameter in pt (smaller)
    const stx = 432 - ss/2;        // centre x ~432 (right side, above QR)
    const sty = 542;               // top y (above the QR which starts ~618)
    ctx.save();
    ctx.globalAlpha = 0.9;
...
/* Right-side signatory signature (optional). The dropdown value is an
   assessor name whose signature image is reused; empty value = no signature. */
function currentManagerSig(){
  if (!F.managerSig || !F.managerSig.value) return null;
  const a = ASSESSORS.find(x=>x.name===F.managerSig.value);
  if(!a) return null;
  return sigCache[a.file] || null;
}
...
async function exportPDF(){
  ...
  // Optional right signatory signature (signature only)
  const rsig=currentManagerSig();
  if (rsig){
    const box={x:451,y:628,w:94,h:46};
    const ir=rsig.width/rsig.height, br=box.w/box.h; let dw,dh;
    if(ir>br){dw=box.w;dh=box.w/ir;}else{dh=box.h;dw=box.h*ir;}
    const sc=document.createElement('canvas'); sc.width=rsig.width; sc.height=rsig.height;
    sc.getContext('2d').drawImage(rsig,0,0);
    doc.addImage(sc.toDataURL('image/png'),'PNG', box.x+(box.w-dw)/2, box.y+(box.h-dh)/2, dw, dh);
  }
  ...
  // QR (smaller) centred at x~404
  if (assets.qr){
    const qs=70, qx=404-qs/2, qy=618;
    ...
    doc.addImage(qc.toDataURL('image/png'),'PNG',qx,qy,qs,qs);
  }
  ...
  if (assets.stamp){
    ...
    doc.addImage(tc.toDataURL('image/png'),'PNG',stx,sty,ss,ss);
    if (doc.setGState) { try { doc.setGState(new doc.GState({opacity:1})); } catch(e){} }
  }
...
if (F.managerSig) F.managerSig.addEventListener('change', async ()=>{
  if (F.managerSig.value) await ensureSig(F.managerSig.value);
  refresh();
});
...
(async function init(){
  ...
  try { assets.stamp = await loadImage('assets/tuv_stamp.png'); } catch(e){ console.warn('stamp missing',e); }
  try { await ensureSig(ASSESSORS[0].name); } catch(e){}
  try { if (F.managerSig && F.managerSig.value) await ensureSig(F.managerSig.value); } catch(e){}
