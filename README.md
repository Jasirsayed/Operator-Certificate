...
- **Asst Manager / Team Lead.** The right-hand signatory shows either “Asst Manager” or “Team Lead”, with a **signature** chosen from the same 7-assessor list (defaults to a signature; choose “None” to omit it).
- **TÜV stamp** is overlaid automatically in the upper-right of the signatory area, matching the official certificate.
...
> **Important:** open the site through a small web server — **do not** double-click `index.html`. When opened as a `file://` page, the browser blocks loading the background, stamp, and signature images, so they will appear **missing**. Serving over HTTP (below) or on GitHub Pages fixes this.
